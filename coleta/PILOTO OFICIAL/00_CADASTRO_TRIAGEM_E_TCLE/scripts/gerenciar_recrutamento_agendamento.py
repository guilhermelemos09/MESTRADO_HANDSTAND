...
def mapear_dados_form1(row):
    ...
    nome_low = normalizar_texto(res.get("Nome_Completo", "")).lower()
    if "vinicius paulo hilario" in nome_low:
        res["Criterio_Triagem_HS"] = "Apenas sustentar na parada de mãos (>= 3s)"
        res["Observacao_Validada"] = "Aluno do pesquisador (Guilherme): sustenta 3s livre com segurança; HSW ainda em desenvolvimento."
...
def sincronizar_formularios():
    ...
    for n_lower in todos_nomes_forms:
        ...
        if d1.get("Observacao_Validada"):
            obs_itens.append(f"Nota: {d1['Observacao_Validada']}")
        if membro_dom:
